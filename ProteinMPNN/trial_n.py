...
for i in tqdm(range(0, correct_idx.shape[0])):
    ...
    # 保存当前帧为图像文件（可选）
    image_file = "output_frames/frame_{}".format(frame_list[i])
...
# cmd.movie.produce("myy_1000_20")

# cmd.mencoder('output_video.mp4', 'output_frames/frame_*.png', format='mp4', mode=1, rate=1)
